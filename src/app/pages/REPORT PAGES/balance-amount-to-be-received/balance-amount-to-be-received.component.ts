...
import { DxDataGridComponent, DxTreeViewComponent, DxLookupComponent, DxButtonModule, DxDataGridModule, DxDropDownButtonModule, DxSelectBoxModule, DxTextBoxModule, DxLookupModule, DxResizableModule, DxDropDownBoxModule, DxFormModule, DxDateBoxModule, DxToolbarModule, DxAccordionModule, DxCheckBoxModule, DxSliderModule, DxTagBoxModule, DxTemplateModule, DxPopupModule, DxTreeViewModule, DxSortableModule, DxTabPanelModule, DxListModule, DxValidatorModule, DxValidationSummaryModule, DxLoadPanelModule, DxTabsModule } from 'devextreme-angular';
...
import { ClaimSummarypayerWiseDrillDownModule } from '../../REPORT DRILL PAGES/claim-summary-payer-wise-drill-down/claim-summary-payer-wise-drill-down.component';
import { ClaimDetailActivityDrillDownModule,ClaimDetailActivityDrillDownComponent } from '../../REPORT DRILL PAGES/claim-detail-activity-drill-down/claim-detail-activity-drill-down.component';

@Component({
  selector: 'balance-amount-to-be-received',
  templateUrl: './balance-amount-to-be-received.component.html',
  styleUrl: './balance-amount-to-be-received.component.scss',
  providers:[ReportService, ReportEngineService, DatePipe, DataService]
})
export class BalanceAmountToBeReceivedComponent implements OnInit {
  ...
  @ViewChild(DxDataGridComponent, { static: false })
    ...
    @ViewChild(ClaimSummarypayerWiseDrillDownModule, { static: false })
    claimSummaryPayerWiseDrill: ClaimSummarypayerWiseDrillDownModule;
  
    @ViewChild('lookup', { static: false }) lookup: DxLookupComponent;
  
    //=================DataSource for data Grid Table========
    dataGrid_DataSource: DataSource<any>;
    BalanceAmt_dataGrid_DataSource: DataSource<any>;
    InitialNotRemittanceBalance_dataGrid_DataSource: DataSource<any>;
    AfterResubmissionNotRemittedBalance_dataGrid_DataSource : DataSource<any>;
  
    columnsConfig: any; //==============Column data storing variable
    payercolumnsConfig : any;
    receivercolumnsConfig : any;
    BalanceAmtcolumnsConfig : any;
    InitialNotRemittanceBalanceConfig:any;
    AfterResubmissionNotRemittedBalanceConfig:any;
    ...
    BasedOnInitialNetAmount : boolean =false;
    ...
    maxDate: Date;
    ColumnNames: any;
    AfterResubmissionNotRemittedBalanceColumnNames:any;
    receiverColumnNames:any;
    BalanceAmtColumnNames:any;
    InitialNotRemittanceBalanceColumnNames:any;
    ...
    drilldownPopups: any[];
    isDrillOpened:boolean =false;
    InnerClickedRowData:any;
    //============Custom close button for drilldown popup============
    toolbarItems: any;

    tabs = [
      { id: 0, text: 'Balance Amount To Be Received' },
      { id: 1, text: 'Initial Not Remitted Balance' },
      { id: 2, text: 'After Resubmission Not Remitted Balance' }
    ];
    ...
    handleRowDrillDownClick = (e: any) => {
      ...
      this.isDrillOpened = true;
      this.popupStateService.setPopupState(
        'ClaimSummaryPayerWiseInnerDrillDown',
        true
      );
    };
    ...
    async get_Datagrid_DataSource() {
      ...
      try {
        const response: any = await this.service
          .fetch_Balance_Amount_To_Be_Received_Data(formData)
          .toPromise();
        if (response.flag === '1') {
          
          this.isEmptyDatagrid = false;
          // this.detailData = response.detail;
          this.columndata = response.AfterResubmissionNotRemittedBalance.ReportColumns;
          ...
          this.InitialNotRemittanceBalanceConfig = this.generateColumnsConfig(
            response.InitialNotRemittanceBalance.ReportColumns,
            userLocale
          );
          this.AfterResubmissionNotRemittedBalanceConfig = this.generateColumnsConfig(
            response.AfterResubmissionNotRemittedBalance.ReportColumns,
            userLocale
          );
          ...
          this.AfterResubmissionNotRemittedBalanceColumnNames = this.AfterResubmissionNotRemittedBalanceConfig
            .filter((column) => column.visible)
            .map((column) => column.caption);

          this.BalanceAmtColumnNames = this.BalanceAmtcolumnsConfig
            .filter((column) => column.visible)
            .map((column) => column.caption);

          this.InitialNotRemittanceBalanceColumnNames = this.InitialNotRemittanceBalanceConfig
            .filter((column) => column.visible)
            .map((column) => column.caption);
          ...
          const ReportData1 = response.BalanceAmt.ReportData;
          const ReportData2 = response.InitialNotRemittanceBalance.ReportData;
          const ReportData3 = response.AfterResubmissionNotRemittedBalance.ReportData;
  
          // Initialize dataGrid_DataSource with the pre-loaded data
          this.BalanceAmt_dataGrid_DataSource = new DataSource<any>({
            load: () => Promise.resolve(ReportData1),
          });

          this.InitialNotRemittanceBalance_dataGrid_DataSource = new DataSource<any>({
            load: () => Promise.resolve(ReportData2),
          });
          this.AfterResubmissionNotRemittedBalance_dataGrid_DataSource = new DataSource<any>({
            load: () => Promise.resolve(ReportData3),
          });
    ...
    generateSummaryColumns(reportColumns) {
      ...
      const uniqueColumns = reportColumns.filter(
        (col, index, self) => index === self.findIndex(c => c.Name === col.Name)
      );
    
      const decimalColumns = uniqueColumns.filter(
    ...
    //============Show Filter Row==========================
    filterClick = () => {
      if (this.BalanceAmt_dataGrid_DataSource) {
    ...
    onExporting(event: any) {
      const fileName = 'Cliam-Summary-Payerwise';
...
@NgModule({
  imports: [
    ...
    ClaimSummarypayerWiseDrillDownModule,
    DxTabsModule,
    ClaimDetailActivityDrillDownModule
  ],
  providers: [],
  exports: [],
  declarations: [BalanceAmountToBeReceivedComponent],
})
export class BalanceAmountToBeReceivedModule {}
