...
import {
  ClaimDetailActivityDrillDownComponent,
  ClaimDetailActivityDrillDownModule,
} from '../../REPORT DRILL PAGES/claim-detail-activity-drill-down/claim-detail-activity-drill-down.component';
...
@Component({
  ...
  styleUrls: ['./claim-analysis.component.scss'],
...
export class ClaimAnalysisComponent implements OnInit {
  ...
  bandColorsMap = {}; // Store band colors globally
  columnToBandMap = {}; // Map child columns to their parent band
  ...
  get_searchParameters_Dropdown_Values() {
    ...
    //===other parameter values loading===
    this.service.get_Claim_Analysis_SearchParametrs_Data().subscribe(
      (response: any) => {
        if (response.flag == '1') {
  ...
  async get_Datagrid_DataSource() {
    ...
    try {
      const response: any = await this.service
        .fetch_Claim_Analysis_Main_Data(formData)
        .toPromise();
      if (response.flag === '1') {
        ...
        this.dataGrid_DataSource = new DataSource<any>({
          load: () => Promise.resolve(response.Summary),
        });
  ...
  generateColumnsConfig(reportColumns, userLocale) {
    const bandedColumns = {};
    const normalColumns = [];
    const bandColors = {}; // Store unique colors for each band
    const columnToBand = {}; // Store child column -> band mapping
    const colorPalette = [
      '#FFB6A6', // Light Coral
      '#A7E9AF', // Soft Green
      '#A6C9FF', // Soft Blue
    ...
    return [...Object.values(bandedColumns), ...normalColumns];
  }
...
export class ClaimAnalysisModule {}
