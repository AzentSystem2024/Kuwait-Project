...
@Component({
  selector: 'app-payer-wise-report',
  templateUrl: './payer-wise-report.component.html',
  styleUrl: './payer-wise-report.component.scss',
  providers:[ReportService, ReportEngineService, DatePipe, DataService]
})
export class PayerWiseReportComponent implements OnInit {
  ...
  @ViewChild(DxDataGridComponent, { static: false })
    ...
    payer_dataGrid_DataSource: DataSource<any>;
    receiver_dataGrid_DataSource: DataSource<any>;
    ...
    tabs = [
      { id: 0, text: 'Payer wise' },
      { id: 1, text: 'Receiver wise' }
    ];
    ...
    async get_Datagrid_DataSource() {
      ...
      try {
        const response: any = await this.service
          .fetch_Claim_Summary_Payer_Wise(formData)
          .toPromise();
        if (response.flag === '1') {
          
          this.isEmptyDatagrid = false;
          this.detailData = response.detail;
          this.columndata = response.payerwise.ReportColumns;
          ...
          this.payerColumnNames = this.payercolumnsConfig
            .filter((column) => column.visible)
            .map((column) => column.caption);

          this.receiverColumnNames = this.receivercolumnsConfig
            .filter((column) => column.visible)
            .map((column) => column.caption);
          ...
          const ReportData1 = response.payerwise.ReportData;
          const ReportData2 = response.receiverwise.ReportData;
  
          // Initialize dataGrid_DataSource with the pre-loaded data
          this.payer_dataGrid_DataSource = new DataSource<any>({
            load: () => Promise.resolve(ReportData1),
          });

          this.receiver_dataGrid_DataSource = new DataSource<any>({
    ...
    //============Show Filter Row==========================
    filterClick = () => {
      if (this.payer_dataGrid_DataSource) {
    ...
    save_Memorise_Report() {
      ...
      const allColumns = this.payerColumnNames;
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [PayerWiseReportComponent],
})
export class PayerWiseReportModule {}
